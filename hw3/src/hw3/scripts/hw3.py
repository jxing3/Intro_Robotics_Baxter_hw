...
class MoveArm(object):
    ...
    def motion_plan(self, q_start, q_goal, q_min, q_max):
        ...
        q_list = self.get_RRt(q_start, q_goal, q_min, q_max)
        tx = "RRT path has " + str(len(q_list)) + " points"
        print tx
        q_list = self.shortcutting_path(q_list)
        tx = "Short-cuted path has " + str(len(q_list)) + " points"
        print tx
        q_list = self.resample_path(q_list,0.5)
        #print q_list
        ...
        v_list,a_list,t,coeff = self.trajectory_plan(q_list)

        if(self.show_plots):
            self.plot_trajectory(len(q_list)-1, coeff[0], 1)
    ...
    def shortcutting_path(self,q):
        ...
        while True:
            path_aux = []
            # Check is you can connect node to goal
            qs = q_new[-1]
            path_aux.append(qs)
            path_aux.append(q_goal)
            ind,valid = self.is_path_valid(path_aux,0,0)
            if valid:
                q_new.append(q_goal)
                break
            for val in reversed(q):
                path_aux = []
                # Check is you can connect node to farthest node
                path_aux.append(qs)
                path_aux.append(val)
                ind,valid = self.is_path_valid(path_aux,0,0)
                if valid:
    ...
    def trajectory_plan(self, q_list):
        ...
        coeff_arr= [];
        for i in range(0,len(q_list[0])):
            temp = [];
            for j in range(0,len(q_list)):
                temp.append(q_list[j][i]); 
            coeff_arr.append(self.coeff_cal(temp));
        ...
        for i in range(0,len(q_list)-1):
            temp_v = [];
            temp_a = [];
            for j in range(0,len(q_list[0])):
                (q,a,v) = self.sample_polynomial(coeff_arr[j],i,0);
                temp_v.append(v)
                temp_a.append(a)
        
            v_list.append(numpy.array(temp_v));
            a_list.append(numpy.array(temp_a));
        ...
        for j in range(0,len(q_list[0])):
            ...
            (q,a,v) = self.sample_polynomial(coeff_arr[j],len(q_list)-2,1);
            temp_v.append(v)
            temp_a.append(a)
        ...
        return v_list,a_list,t,coeff_arr


    def coeff_cal(self, q):
    ...
    def resample_path(self,q,thr):
        q_new = []
        for j in range(0,len(q)-1):
            qs = q[j]
            qg = q[j+1]
            path_aux = self.create_path_with_thr2(qs,qg,thr)
            path_aux.pop()
            q_new = q_new + path_aux
        q_new.append(q[-1])
        return q_new
    def get_RRt(self, q_start, q_goal, q_min, q_max):
        #
        no_list = []
        n = RRTNode()
        n.q = q_start
        ...
        while True:
            ...
            path_aux.append(n.q)
            path_aux.append(q_goal)
            ind,valid = self.is_path_valid(path_aux,q_min,q_max)
            if valid:
                ...
                no_list.append(n2) 
                break
            # if you cannot connect, create new node
            q_aux = numpy.zeros(7)
            c = 0
            for j in q_min:
                q_aux[c]=numpy.random.rand(1)*(q_max[c] - j) + j
                c+=1
            mag = 1000000
            # get closest node
            for node in no_list:
                mag_aux = self.mag_vec(q_aux-node.q)
                if mag_aux<mag:
                    mag = mag_aux
                    # save closest node
                    n_aux = node
            q_aux = q_aux/numpy.max(q_aux) * 0.5
            #check is node is valid
            path_aux = []
            path_aux.append(n_aux.q)
            q_aux_lim = self.q_with_limits(n_aux.q + q_aux,q_min,q_max)
            path_aux.append(q_aux_lim)
            ind,valid = self.is_path_valid(path_aux,q_min,q_max)
            if valid:
                n = RRTNode()
                n.q = q_aux_lim
                ...
                no_list.append(n)
            count += 1
            tx = "Nodes inserted: " + str(count)
            print tx
            # check if limit is reached
            if count > 2000:
                q_list=[]
                q_list.append(q_start)
                print "Limit nodes reached"
                return q_list

        # work back the tree to create path
        q_list = []
        while True:
            q_list.insert(0,n2.q)
            n_aux = n2.parent
            if n_aux == 0:
                break
            n2 = n_aux
        return q_list
    def q_with_limits(self,q,q_min,q_max):
        c = 0
        q_new = q*0
        for k in q:
            aux = k
            ...
            c+=1
        return q_new
    def is_path_valid(self,q,q_min,q_max):
        ## check for joint limits
        es = True
        for j in range(0,len(q)-1):
            qs = q[j]
            qg = q[j+1]
            path_aux = self.create_path_with_thr(qs,qg,0.1)
            for p in path_aux:
                es = self.is_state_valid(p)
                if not(es):
                    return j,es
            j += 1
        return -1,es

    def create_path_with_thr2(self,q_start,q_goal,thr):
        # Receives start and end and returns a path of N values, each thr apart or more for last element
        new_path = []
        new_path.append(q_start)
        count = 0
        par = True
        while par:
            c_node = new_path[-1]
            vec = q_goal - c_node
            mag = self.mag_vec(vec)
            if mag <= thr:
                #new_path.append(c_node+vec)
                break
            new_path.append(c_node+(vec/mag*thr))
        new_path.pop
        new_path.append(q_goal)

        return new_path
    def create_path_with_thr(self,q_start,q_goal,thr):
        # Receives start and end and returns a path of N values, each thr apart or less for last element
        new_path = []
        new_path.append(q_start)
        count = 0
        par = True
    ...
    def mag_vec(self,vec):
        # This function gets a vector and returns its magnitude
        s = 0
        for val in vec:
            s = s+val*val
        mag = math.sqrt(s)
